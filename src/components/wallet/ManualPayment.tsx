...
import { FaPiggyBank, FaUserFriends } from 'react-icons/fa'
...
const ManualPayment = ({setPaymentType}: Props) => {
    ...
    const handleContinue = () => {
        if(step === 1){
            ...
            // check is bank
            if(selectedOption === "bank"){
//  check if userBankInfo is fullfield
    ...
    const STEP_2=(<div>
        {
            selectedOption === "bank" ? <div>
                <div className='flex flex-col items-center justify-center w-full bg-orange-50 py-4 rounded'>
                <p className='text-2xl font-bold flex gap-2 items-center'> <FaNairaSign /> {amount*config.dollarRate}</p>
                ...
                <div className='space-y-2 mt-4' >
                    ...
                    <div className=' py-2'>
                        <p className='text-sm text-textGrey'> <span className='text-orange-500'>*</span> By clicking continue, you agree that you have sent the money to the selected bank.</p>
                    </div>
                </div>
            </div> : <div>
  ...
  return (
    <div className='w-[500px] space-y-4 pt-3'>
        {
            step === 1 ? STEP_1 : step === 2 ? STEP_2 : STEP_3
        }
