...
import { BsCashCoin, BsBank2 } from "react-icons/bs";
...
export default function AddMoneyModal() {
  ...
  const [addRequest, { isLoading }] = useAddCurrencyRequestMutation();
  const [addRequestWithPayStack, { isLoading: isPayStackLoading }] = useAddCurrencyRequestWithPayStackMutation();
  const [addRequestWithKoraPay, { isLoading: isKoraPayLoading }] = useAddCurrencyRequestWithKoraPayMutation();
  ...
  const handlePay = () => {
    if (selectedOption === "crypto") {
      if (amount < config.fundBtcMinMoney) {
        return toast.error(`Minimum amount is ${config.fundBtcMinMoney}`, { toastId: 1 });
      }
    } else if (amount < config.fundMinMoney) {
      return toast.error(`Minimum amount is ${config.fundMinMoney}`, { toastId: 1 });
    }
  ...
  return (
    ...
    >
      {!paymentType ? (
        <div>
          <div>
            {/* Render deposit options */}
            <div className="flex mt-5 flex-col md:flex-row md:w-[400px] gap-2 md:gap-5">
              <button
                className="p-2 py-4 md:p-4 border rounded-lg transition-all border-borderColor w-full max-w-sm text-left group hover:border-orange-400"
                onClick={() => setPaymentType("online")}
              >
                <div>
                  <div className="flex justify-center items-center">
                    <BsCashCoin className="size-10 group-hover:text-orange-400 transition-all" />
                  </div>
                  <div>
                    <h3 className="text-center text-textBlack mt-2 group-hover:text-orange-400 font-bold transition-all">
                      Quick Deposit
                    </h3>
                    <p className="text-xs md:text-sm text-center text-textGrey">
                      Instant funding via Bank Transfer, Debit/Credit Card, or Crypto.
                    </p>
                  </div>
                </div>
              </button>
              {/* Only render Manual Deposit button if manual deposit is active */}
              {config.isManualDepositActive && (
                <button
                  className="gap-5 p-2 py-4 md:p-4 border rounded-lg transition-all w-full max-w-sm text-left group hover:border-orange-400 border-borderColor"
                  onClick={() => setPaymentType("manual")}
                >
                  <div className="w-full">
                    <div className="flex justify-center items-center">
                      <BsBank2 className="size-10 group-hover:text-orange-400 transition-all" />
                    </div>
                    <div>
                      <h3 className="text-textBlack text-center mt-2 group-hover:text-orange-400 font-bold transition-all">
                        Manual Deposit
                      </h3>
        ...
        <ManualPayment setModalOpen={setModalOpen} setPaymentType={setPaymentType} />
      ) : (
        <div>
          <div className="space-y-4 pt-4 md:w-[520px]">
            ...
            {selectedOption === "bank-kora" && (
              <p className="text-sm text-textGrey mt-2">
                <strong>Note:</strong> Transaction charges for bank deposits or card payments, as
                directed by CBN, are to be covered by the customer.
              </p>
            )}
            {selectedOption === "bank" && (
              <p className="text-sm text-textGrey mt-2">
            ...
            {selectedOption === "crypto" && (
              <p className="text-sm text-textGrey mt-2">
                ...
                underpayment will lead to insufficient funding.
              </p>
            )}
            <button
              onClick={() => {
            ...
            >
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-textGrey">
                  Fund your wallet with popular cryptocurrencies like USDT, ETH, BNB, SOL and more.
                </p>
            ...
            <div className="flex mt-4 gap-3">
              ...
              >
                {isLoading || isPayStackLoading || isKoraPayLoading || isOxLoading ? "Loading" : "Continue"}
              </button>
            </div>
          </div>
        </div>
