...
const PriceRange = () => {
  ...
  const handleMinPriceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      ...
      maxPrice - newMinPrice >= priceGap &&
      newMinPrice >= lowestMinPrice
    ) {
  ...
  const handleMaxPriceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      ...
      newMaxPrice - minPrice >= priceGap &&
      newMaxPrice <= highestMaxPrice
    ) {
  ...
  return (
    <div className="pt-1">
      <div className="h-2 md:h-2.5 bg-red/15 rounded mt-2 relative">
        <div
          className="h-full bg-primary rounded absolute"
          style={{
            left: `${(minPrice / highestMaxPrice) * 100}%`,
            right: `${100 - (maxPrice / highestMaxPrice) * 100}%`,
          }}
        />
      </div>
      ...
      <div className="flex items-center gap-4 mt-6">
        <div className="flex-1 border border-borderColor rounded-lg px-2 md:px-3 lg:px-4 py-1 2xl:py-1.5 outline-none">
          <label className="text-darkishGrey text-xs">Minimum</label>
        ...
        <div className="flex-1  border border-borderColor rounded-lg px-2 md:px-3 lg:px-4 py-1 2xl:py-1.5 outline-none">
          <label className="text-darkishGrey text-xs">Maximum</label>
          <div className="flex items-center">
            <PiCurrencyDollarBold />
            <input
              type="number"
