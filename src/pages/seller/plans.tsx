  ...
  const handleTakeAPlan = async (value: string) => {
    ...
    if (currentPlan?.data?.days > 0) {
      return toast.error(
        ...
        } and after ${getDaysRemaining(
          currentPlan?.data?.createdAt
        )} days you can change your plan!`,
        { toastId: 1 }
      );
    }

    await takeAPlan({ planType: value })
      .unwrap()
        ...
        <section className="min-h-[90dvh] layout">
          {currentPlan?.data?.planType !== "default" && (
            <div className="border-primary/50 bg-primary/10 flex flex-wrap gap-2 mx-auto w-fit rounded-lg border-l-4 2xl:border-l-[6px] p-3 md:p-4">
              <div className="">
                <GoAlert className="text-primary/50 text-xl inline" /> You are
                in{" "}
                <span className="font-bold inline">
                  {(currentPlan?.data?.planType === "default" &&
                    "DEFAULT PLAN") ||
                    (currentPlan?.data?.planType === "basic" && "BASIC PLAN") ||
                    (currentPlan?.data?.planType === "pro" &&
                      "BUSINESS PLAN") ||
                    (currentPlan?.data?.planType === "proPlus" && "PRO PLAN")}
                  .
                </span>
          ...
          <h2 className="text-center text-2xl lg:text-4xl font-bold mt-7">
            Upload Packages and pricing
          </h2>
            ...
            {plans.map((plan) => (
              ...
              >
                <div className="text-center">
                  <h2
                    className={`text-3xl font-bold uppercase ${
                      plan.name === active ? "text-white" : "text-textBlack"
                    }`}
                  >
                    {plan.name}
                  </h2>
                  ...
                  <h4
                    className={`font-bold text-4xl ${
                      plan.name === active ? "text-white" : "text-primary"
                    }`}
                  >
                    ${plan.price}
                  </h4>
                ...
                {isLoading ? (
                  ...
                  <button
                    onClick={() => handleTakeAPlan(plan.name)}
